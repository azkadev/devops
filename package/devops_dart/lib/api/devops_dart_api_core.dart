...
import 'package:devops/api/extensions.dart';
import 'package:devops_scheme/scheme/scheme/devops_pubspec_config.dart';
import 'package:devops_scheme/scheme/scheme/pubspec_devops.dart';
import 'package:devops_scheme/scheme/scheme/pubspec_devops_dependencies.dart';
...
class DevopsDartApi {
  ...
  FutureOr<dynamic> create({
    ...
    PubspecDevops pubspecDevops = PubspecDevops(yaml_code.clone());

    File file_guide = File(path.join(directory_project.path, "guide-devops.md"));

    await file_guide.writeAsString(guide_devops_markdown());
    ...
    if (!file_script_example.existsSync()) {
      // await file_script_example.writeAsString(script_devops_native());
    }
    ...
    PubspecDevops pubspecDevops_default = PubspecDevops.create(
      repository: "https://github.com/{main_username_owner}/devops",
      homepage: "https://github.com/{main_username_owner}/devops",
      issue_tracker: "https://github.com/{main_username_owner}/devops/issues",
      documentation: "https://github.com/{main_username_owner}/devops/tree/main/docs",
      funding: [
        "https://github.com/sponsors/{main_username_owner}",
      ],
      dependencies: PubspecDevopsDependencies({
        "devops": "any",
        "devops_http_client": "any",
        "general_lib": "^0.0.34",
      }),
      devops: DevopsPubspecConfig.create(),
    );

    // update pubspec default
    pubspecDevops.rawData.devops_updateMapIfNotSameOrEmptyOrNull(
      data: pubspecDevops_default.toJson(),
      ignoreKeys: [
        "@type",
      ],
    );
    String yaml_documents_new = YamlWriter().write(pubspecDevops.toJson());
  ...
  FutureOr<int> run({
    required Directory directoryBase,
    required Directory? directoryOutputBuildDevops,
    required String? inputFileName,
    required DevopsDartBuildType server_universeDartBuildType,
  }) async {
    if (Dart.isWeb) {
      return 1;
    }
  ...
  FutureOr<dynamic> build({
    ...
    Map yaml_code = (yaml.loadYaml(file_pubspec.readAsStringSync(), recover: true) as Map);
    PubspecDevops pubspec_server_universe = PubspecDevops(yaml_code.clone());
  ...
  static String guide_devops_markdown() {
    return r"""
